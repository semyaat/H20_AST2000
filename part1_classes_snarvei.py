...
random.seed(30) #Setter en fast verdi til del 5
...
class Rocket_launch(Calculations):
    ...
    def simulate_rocket_launch(self, N, init_fuel, no_boxes, max_time, ant_iter, r0, v0):
        #Oppgave 1E
        ...
        #Initialbetingelser
        # v_tangent = (2*np.pi/self.rot_per)*plan_radii        #Regner ut hastigheten i vx-retningen basert på planetens rotasjon
        # #v0 = np.array([0,v_tangent + plan_vel])                    #Initialhastighet er vx i x-retning og 0 i y-retning
        # v0 = np.array([0, v_tangent])
    ...
    def set_parameters_and_verify(self):
        ...
        launch_duration = self.t[-1] 
        rocket_position_before_launch = self.rocket_position_before_launch
        # rocket_position_after_launch = self.rocket_position_after_launch #Trenger ikke denne?
        time_of_launch = 0
        
        mission.set_launch_parameters(thrust_per_box_times_number_of_boxes, mass_loss_rate_per_box_times_number_of_boxes, initial_fuel_mass, launch_duration, rocket_position_before_launch, time_of_launch)
...
if __name__ == '__main__':
    ...
    launch.simulate_rocket_launch(100000,24000,10**(12),1200,1000)
